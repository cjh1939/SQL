...
# 단, GROUP BY 절에 작성한 컬럼은 단일행함수와 함께 사용가능
SELECT MAX(SAL), ENAME FROM emp;


# 조회 데이터 그룹화하기 
# 모든 사원 정보에서 가장높은 급여를 조회하시오.
SELECT MAX(SAL) FROM emp;

# 모든 사원 정보에서 부서별로 가장높은 급여를 조회하시오.
...
# 부서별 커미션의 평균을 조회 
#단, 10번부서는 제외하고, 평균은 전체사원수를 기준으로 조회한다.
...
# 직급별 최소 급여를 조회하되 , 사원직급은 제외하며 
# 직급별 최소 급여가 300 미만인 데이터로 제외한다.
# 조회 결과 데이터를 직급기준 오름차순으로 정렬 하세요.
